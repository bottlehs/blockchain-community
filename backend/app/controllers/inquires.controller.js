...
exports.create = (req, res, next) => {
  ...
  Inquire.create(inquire)
    ...
    .catch((err) => {
      res.status(500).send({
        message:
          err.message || "Some error occurred while creating the Inquire.",
      });
    });
};
...
exports.findAll = (req, res) => {
  ...
  Inquire.findAndCountAll({ where: condition })
    .then((data) => {
    ...
    .catch((err) => {
      res.status(500).send({
        message:
          err.message || "Some error occurred while retrieving inquires.",
      });
    });
};
...
exports.deleteAll = (req, res) => {
  Inquire.destroy({
    ...
    .catch((err) => {
      res.status(500).send({
        message:
          err.message || "Some error occurred while removing all inquires.",
      });
    });
};
